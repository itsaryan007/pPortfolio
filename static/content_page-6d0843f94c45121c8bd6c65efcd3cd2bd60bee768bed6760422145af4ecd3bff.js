...
(function ($) {
  ...
  if (
    !$.event.special.focusin &&
    !$.event.special.focusout &&
...
(function () {
  ...
  this.ModalMessage = function (options) {
    ...
    html +=
      '<div class="_4ORMAT_overlay"></div><div class="ModalMessage generated ' +
      modal_class +
      '">';
    ...
    $(document)
      .unbind("keydown.modal")
      .bind("keydown.modal", function (e) {
        if (e.which == 13) {
          if (!options.enter_button) {
            options.enter_button = "close";
          }
    ...
    if (options.prevent_close !== true) {
      $("html").delegate(
        ".ModalMessage .close, ._4ORMAT_overlay",
        "click.modal_close",
        this.close_modal
      );
    }
...
(function () {
  ...
  function beginFormInit() {
    if (typeof _4ORMAT.contact_forms === "object") {
      $(".email_form[data-editable-type='contact-form']").each(function (
        _i,
        form
      ) {
        var $form = $(form);
  ...
  ContactForm.initForm = function (form) {
    if (
      _4ORMAT_DATA &&
      !_4ORMAT_DATA.page.content_editable &&
      !_4ORMAT_DATA.preview
    ) {
      $.ajax("/public_heartbeat").fail(function () {
        form.find("input, textarea").prop("disabled", true);
        form
          .find("input[type=submit]")
          .attr("originalValue", form.find("input[type=submit]").val());
        form
          .find("input[type=submit]")
          .val("Form is unavailable, please try again later");
      });
    }
  ...
  ContactForm.errorPlacement = function (error, element) {
    $(element).tooltip("destroy").attr("title", $(error).text());
    $(element)
      .tooltip({
        html: false,
        placement: "bottom",
  ...
  ContactForm.saveSubmitValue = function (form) {
    var formSubmitValue = $(form).find("input[type=submit]").val();
  ...
  ContactForm.sendSuccess = function () {
    ...
    $(this).find("fieldset").removeClass("_4ORMAT_module_input_active");
  };
